...
export const createSession = createAsyncThunk(
  "interactions/createSession",
  async ({ sessionId, title }, { rejectWithValue }) => {
    try {
      const { data } = await axios.post(`${API_BASE_URL}/sessions`, {
...
export const fetchSessionMessages = createAsyncThunk(
  "interactions/fetchSessionMessages",
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(postChatMessage.fulfilled, (state, action) => {
        ...
        const sessionId =
          action.payload.session_id ?? action.payload.sessionId ?? null;
        if (sessionId) {
          ...
          if (
            structuredResponse?.form_updates &&
            typeof structuredResponse.form_updates === "object"
          ) {
            const updates = normalizeIncomingAiUpdates(
              structuredResponse.form_updates,
            );
            Object.entries(updates).forEach(([field, value]) => {
              if (!Object.prototype.hasOwnProperty.call(state.formDraft, field))
