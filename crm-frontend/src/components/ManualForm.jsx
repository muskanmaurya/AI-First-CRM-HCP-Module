...
const buildSummary = (draft) => {
  const lines = [];
  if (draft.topicsDiscussed)
    lines.push(`Topics Discussed: ${draft.topicsDiscussed}`);
  if (draft.attendees.length)
    lines.push(`Attendees: ${draft.attendees.join(", ")}`);
  if (draft.materialsShared.length)
    lines.push(`Materials Shared: ${draft.materialsShared.join(", ")}`);
  if (draft.samplesDistributed.length)
    lines.push(`Samples Distributed: ${draft.samplesDistributed.join(", ")}`);
  ...
  if (draft.followUpActions)
    lines.push(`Follow-up Actions: ${draft.followUpActions}`);
    ...
    {value.length > 0 && (
      <div className="flex flex-wrap gap-2 pt-1">
        {value.map((item, index) => (
          <span
            key={`${item}-${index}`}
            className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-medium text-slate-700 shadow-sm"
          >
            {item}
            <button
              type="button"
...
const ManualForm = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      Object.values(highlightTimeoutsRef.current).forEach((timerId) =>
  ...
  const addMaterial = () => {
    dispatch(
      addFormListItem({ field: "materialsShared", value: materialDraft }),
    );
    setMaterialDraft("");
  };
  ...
  const addSample = () => {
    dispatch(
      addFormListItem({ field: "samplesDistributed", value: sampleDraft }),
    );
    setSampleDraft("");
  };
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-slate-50 text-slate-900 flex flex-col">
      {" "}
      <div
        ref={splitContainerRef}
        className={`flex h-full w-full overflow-hidden ${isDesktop ? "flex-row" : "flex-col"}`}
      >
        <div
              ...
              <div className="flex h-full items-center justify-center text-center text-slate-500">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-500">
                    Ask the AI to summarise or structure your interaction note.
                  </p>
                </div>
              </div>
            ) : (
      ...
      {message.role === 'assistant' && message.structured_response?.form_updates && (
        <div className="mt-2 inline-block rounded-full bg-blue-50 border border-blue-100 px-2 py-1 text-xs text-blue-600 font-medium">
          ...
          <div className="flex-none border-t border-slate-200 bg-white px-6 py-5">
            {chatError && (
              <p className="mb-3 text-sm text-red-600">{chatError}</p>
            )}
        ...
        >
          <div className="flex-none border-b border-slate-200 bg-slate-50 px-6 py-5">
            ...
            <p className="text-sm text-slate-600">
              Record structured Life Sciences CRM interaction details
            </p>
          </div>
                  ...
                  <div className="grid gap-4">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className={fieldClass(
                          "hcpName",
                          "w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm shadow-sm outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100",
                        )}
                      />
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="relative">
                            ...
                            <input
                              ...
                              className={fieldClass(
                                "interactionDate",
                                "w-full rounded-2xl border border-slate-200 bg-white py-3 pl-10 pr-4 text-sm shadow-sm outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100",
                              )}
                            />
                          </div>
                          <div className="relative">
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        className={fieldClass(
                          "topicsDiscussed",
                          "w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm shadow-sm outline-none transition placeholder:text-slate-400 focus:border-blue-400 focus:ring-2 focus:ring-blue-100",
                        )}
                      />
                      <button
                        type="button"
                        className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-100"
                      >
                    ...
                    >
                      ...
                      <ChipListEditor
                        ...
                        onRemove={(index) =>
                          dispatch(
                            removeFormListItem({ field: "attendees", index }),
                          )
                        }
                        addLabel="Add"
                    ...
                    >
                      ...
                      <div className="grid gap-3 md:grid-cols-3">
                        {sentimentOptions.map(
                          ({ value, label, icon: Icon, emoji }) => {
                            const selected = formDraft.hcpSentiment === value;
                            return (
                              <label
                                key={value}
                                className={`flex cursor-pointer items-center gap-3 rounded-2xl border px-4 py-3 text-sm shadow-sm transition ${selected ? "border-blue-400 bg-blue-50 ring-2 ring-blue-100" : "border-slate-200 bg-white hover:bg-slate-50"}`}
                              >
                                <input
                                  type="radio"
                                  name="hcpSentiment"
                                ...
                                />
                                <Icon
                                  className={`h-5 w-5 ${selected ? "text-blue-600" : "text-slate-500"}`}
                                />
